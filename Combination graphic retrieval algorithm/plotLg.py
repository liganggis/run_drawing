...
def plot_graph_original_route(
    ...
    **pg_kwargs,
    ...
    x = (tempX[0], tempX[-1])
    y = (tempY[0], tempY[-1])
    ax.scatter(tempX, tempY, s=orig_dest_size, c=route_color, alpha=route_alpha, edgecolor="none")
    ...
    #         x.extend((G.nodes[u]["x"], G.nodes[v]["x"]))
    #         y.extend((G.nodes[u]["y"], G.nodes[v]["y"]))
    # ax.plot(tempX, tempY, c=route_color, lw=route_linewidth, alpha=route_alpha)
...
def plot_graph_routes(G, routes, tempX, tempY, coordsX, coordsY, route_colors="r", route_linewidths=4, **pgr_kwargs):
    """
    ...
    #     **kwargs,
    # )
    # fig, ax = plot_graph_original_route(G,
    #         tempX=tempX, 
    #         tempY=tempY,
    #         route_linewidth=route_linewidths[0],
    #         show=False,
    ...
    # om1= plt.axes([0.01, 0.01, 0.65, 0.03], facecolor=axcolor) # 第一slider的位置

    # som1 = Slider(om1, r'$\omega_1$', 1, 30, valinit=3) # 产生第一slider


    # def updateRoute(ax1, kwargs, r_rc_rlw):
    #     def update(val):
    #         s1 = math.floor(som1.val)
    #         fig, ax = plot_graph_route(
    #             G,
    #             route=r_rc_rlw[s1][0],
    #             route_color='r',
    ...
    # som1.on_changed(updateRoute(ax, kwargs, list(r_rc_rlw)))
...
def plot_graph_routes2(G, paths, route_colors="r", route_linewidths=4, **pgr_kwargs):
    """
    Plot several routes along a graph.
    ...
    if len(paths) < 2:  # pragma: no cover
        raise ValueError("You must pass more than 1 route")
    if isinstance(route_colors, str):
        route_colors = [route_colors] * len(paths)
    if len(paths) != len(route_colors):  # pragma: no cover
        raise ValueError("route_colors list must have same length as routes")
    if isinstance(route_linewidths, int):
        route_linewidths = [route_linewidths] * len(paths)
    if len(paths) != len(route_linewidths):  # pragma: no cover
        raise ValueError("route_linewidths list must have same length as routes")
    ...
    for path in paths:
        ...
        tempY = []
        for coord in path:
            tempX.append(coord[0])
            tempY.append(coord[1])
        tempXs.append(tempX)
        tempYs.append(tempY)
    ...
    for i in range(1, len(tempXs)):
        fig, ax = plot_graph_original_route(
            G,
            tempX=tempXs[i], 
            tempY=tempYs[i],
            route_linewidth=route_linewidths[0],
